...
G4double dzFaWTail1   =  6.00*cm/2;
 ...
 G4double sEnvelopeT_z = (z_0 + 2*dzSteelEnvelopeFC + 2*dzSteelEnvelopeC5 + 2*dzSteelEnvelopeC10 + 2*dzSteelEnvelopeR + 2*dzFaWPlate-2*dzFaWPlateR + 2*dzSteelEnvelopeFI) - mag_position;
...
//  TGeoVolume* voFaSteelEnvelope = new TGeoVolume("AFaSteelEnvelope", shFaSteelEnvelope, kMedSteel);
 ...
 // Logical_volumes.push_back(endplate_Lvolume);
  ...
  G4double endplate_z = (z_0 + 2*dzSteelEnvelopeFC + 2*dzSteelEnvelopeC5 + 2*dzSteelEnvelopeC10 + 2*dzSteelEnvelopeR  + dzEndPlate  - 2*dzFaWPlateR + 2*dzFaWPlate - 2*dzEndPlateI) - mag_position;
 ...
 // endplate_Lvolume->SetVisAttributes(gray);
  ...
  G4VSolid* hole_square0 = new G4SubtractionSolid("Box+Cylinder", endplate_box, endplate_tub1);  
  
  G4VSolid* hole_square = new G4SubtractionSolid("Box+Cylinder1", hole_square0, endplate_tub2, 0,G4ThreeVector(0,0, (-2*dzEndPlateI -0.05*cm)));  
...
// 5 deg cone
  ...
  G4Cons* lead_cons_1 = new G4Cons("lead_cons", rInFaPbCone5, rOuFaPbCone5, 
   ...
   G4double lead_z_1 = (z_0 + 2*dzSteelEnvelopeFC + 2*dzFaWPlate + dzFaPbCone5 - 2*dzFaWPlateR) - mag_position;
  ...
  G4double lead_z = (z_0 + 2*dzSteelEnvelopeFC + 2*dzFaWPlate + 2*dzFaPbCone5 + dzFaPbCone10 - 2*dzFaWPlateR ) - mag_position;
...
G4double kAngle0071 = tan(0.71*PI/180.00);
...
G4Cons* wtail1_cons = new G4Cons("wtail1_cons", rInFaWTube5C1, rOuFaWTube5C1, 
